...
function Product({ id, name, image, price, category, gridView, description }) {
    ...
    return (
        <Link to={`/singleproduct/${id}`}
            className={` md:m-5 lg:m-7  rounded-md overflow-hidden  relative hover:brightness-105  ${gridView ? 'm-1' : 'm-3 flex  border py-2 border-gray-200 flex-col md:flex-row justify-between'}`}
        >
            <figure className={`relative w-full ${gridView ? '' : 'p-3 md:w-4/12 '}`}>
                <img src={image} alt=""
                    className={`rounded-md`}
                />
                {gridView && <figcaption className='flex justify-between py-2 px-3 '>
                    <span>
                        {name}
                    </span>
                    <span>
            ...
            {gridView && <div className="absolute top-3 right-3 bg-white/75 backdrop-blur-md text-neutral-600 uppercase px-3 rounded-full py-1.5 text-sm">
                {category}
            </div>}
